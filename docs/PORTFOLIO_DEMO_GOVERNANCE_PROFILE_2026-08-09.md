# ShopSquire portfolio-demo governance profile
...
- Production tenant or publisher enrollment.
- Independent relevance validation: the current packet is owner-reviewed and remains unsealed until all 43 judgments are entered.
- Production-ready hosted voice, canary operation or observed rollback.
- Buyer-visible LLM narration until both fidelity and three consecutive latency gates pass.
- V2 compatibility endpoint retirement.

## Default adjudication

Pure relevance measures workload fit, not price. Price and availability are separate constraints: an unavailable but relevant configuration may remain visible with an availability penalty and next-best, delayed, split or supplier choices. Architecture alternatives may be shown when they satisfy the outcome better, but mobility requirements prevent a fixed workstation from silently replacing a laptop.

Stale evidence remains visible and conditional with an explicit refresh option. Absence of evidence means **not verified**, never compatible, safe or sufficient.
