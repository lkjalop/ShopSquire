...
def execute(db, *, case_id: str, actor: Actor, idempotency_key: Optional[str] = None,
            today: Optional[str] = None, tenant_id: str = "default", now_iso: Optional[str] = None,
    ...
    quote has expired or the PO is out of the approved scope. Idempotent (one PO per key). SANDBOX ref."""
    cur = workflow.repository.current_version(db, case_id, tenant_id)
    ...
    if not r1.ok and r1.reason != "idempotent_replay":
        return r1
    po_ref = f"PO-{uuid.uuid4().hex[:10].upper()}"
    return workflow.transition(
        db, case_id=case_id, event="purchase_order_created", actor=actor, reason_code="po_created_sandbox",
        idempotency_key=f"{key}-create", evidence={"po_ref": po_ref, "sandbox": True},
        state_patch={"purchase_order": {**po, "status": "created", "po_ref": po_ref, "sandbox": True}},
        tenant_id=tenant_id, now_iso=now_iso, trace_id=trace_id)
