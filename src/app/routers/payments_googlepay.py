...
router = APIRouter(prefix="/api/v1/payments/googlepay", tags=["payments-googlepay"])
tracer = get_tracer("payments-googlepay")
...
def create_intent(
    ...
    with tracer.start_as_current_span("googlepay.create_intent"):
        flags = load_feature_flags(get_settings().feature_flags_path)
        cap = flags.get("CAPABILITIES", {}).get("googlepay", {"enabled": False})
        if not cap.get("enabled"):
            raise HTTPException(status_code=503, detail="Google Pay disabled by feature flags")
        ...
        base_url = os.getenv("GOOGLEPAY_API_BASE_URL", "").strip()
        api_key = os.getenv("GOOGLEPAY_API_KEY", "").strip()
        if not (base_url and api_key):
            raise HTTPException(status_code=503, detail="Google Pay provider not configured")
        payload = {
            "uid": uid,
        ...
        try:
            ...
            if resp.status_code >= 300:
                raise HTTPException(status_code=502, detail={"message": "Google Pay request failed", "provider_status": resp.status_code})
            return {
                "provider": "googlepay",
            ...
            raise HTTPException(status_code=502, detail=f"Google Pay provider unavailable: {exc}")
