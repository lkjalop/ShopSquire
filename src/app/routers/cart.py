...
# NOTE: the old `apply_cart_ops` (op-by-op mutation through the REST handlers) was DELETED
# 2026-07-12 (defect-hunt #5). It had no all-or-nothing guarantee — ops 1-2 could persist then
# op 3 fail, the exact partial-application the C1 boundary exists to prevent. All programmatic
# cart mutation now goes through the transactional, idempotent, stale-guarded
# services.cart_mutation_service.apply_plan (the ONLY mutation path). apply_quantity_line above
# is the shared per-line decision both that service and the PUT handler call.
