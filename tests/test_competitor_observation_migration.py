"""Validate the 20260627_competitor_observation migration: creates the table, idempotent, drift-free
vs the runtime DDL, chains off the catalog-entities head."""
...
_MIG = Path(__file__).resolve().parents[1] / "alembic" / "versions" / "20260627_competitor_observation.py"
_EXPECTED = {"competitor_observation"}


@pytest.fixture(scope="module")
def mig():
    spec = importlib.util.spec_from_file_location("mig_competitor_obs", _MIG)
...
def test_metadata(mig):
    assert mig.revision == "20260627_competitor_observation"
    assert mig.down_revision == "20260626_catalog_entities"
