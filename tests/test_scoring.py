...
def test_scoring_versioning_and_diff_and_rollback():
    ...
    r = client.post("/api/v1/scoring/update", json=payload)
    assert r.status_code == 200
    version = r.json().get("version")
    assert version is not None

    # List versions
    rv = client.get("/api/v1/scoring/versions")
    ...
    if len(versions) >= 2:
        ...
        rd = client.get("/api/v1/scoring/diff", params={"a": a, "b": b})
        assert rd.status_code == 200
        assert "diff" in rd.json()

    # Rollback to the created version
    rr = client.post("/api/v1/scoring/rollback", json={"version_file": versions[-1]})
