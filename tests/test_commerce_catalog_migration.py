"""Validate the 20260626_commerce_catalog migration: creates price_book_entry + inventory_level,
idempotent, drift-free vs the runtime DDL, chains off the supplier-catalog head."""
...
_MIG = Path(__file__).resolve().parents[1] / "alembic" / "versions" / "20260626_commerce_catalog.py"
_EXPECTED = {"price_book_entry", "inventory_level"}


@pytest.fixture(scope="module")
def mig():
    spec = importlib.util.spec_from_file_location("mig_commerce_catalog", _MIG)
...
def test_metadata(mig):
    assert mig.revision == "20260626_commerce_catalog"
    assert mig.down_revision == "20260626_supplier_catalog"  # chains off the prior head
