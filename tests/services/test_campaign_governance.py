...
def _prove_rollback(db, now_iso):
    eid = ex.create_experiment(db, name="x", target_metric="rpv", status="live")
    ops.forced_rollback_drill(db, experiment_id=eid)   # → status reverted
    ops.record_heartbeat(db, now_iso=now_iso)          # eval loop alive
    return eid


def _prove_contact_freq(db):
    cg.evaluate_contact(db, uid_hash="probe", channel="email")  # no consent → a real suppression
...
def test_not_ready_before_anything_demonstrated(db):
    ...
    assert "no_rollback_demonstrated" in r["reasons"]
    assert "contact_governance_not_exercised" in r["reasons"]
...
# ── governed dispatch ─────────────────────────────────────────────────────────
def test_dispatch_blocked_until_ready(db):
    ...
    assert out["blocked_by_readiness"] is True and out["allowed"] == []  # offers can't go out yet


def test_dispatch_honors_contact_governance_once_ready(db):
    _prove_rollback(db, now_iso="2026-06-25 12:00:00")
    _prove_contact_freq(db)
...
def test_dispatch_safe_without_db():
