...
def test_no_qr_returns_none():
    assert _image_security_preamble_note({"qr_code_detected": False}) is None
    assert _image_security_preamble_note({}) is None
    assert _image_security_preamble_note(None) is None


def test_quarantine_note_instructs_model_to_ignore_image_content():
    note = _image_security_preamble_note({"qr_code_detected": True, "qr_payloads": []})
    assert note is not None
    low = note.lower()
