...
test('unresolved open-world workload clears stale commercial state before research', async ({ page }) => {
  ...
  await expect(latest).toContainText(/current external requirements.*approved official sources/i);
  ...
  await expect(research).toContainText(/catalog recommendation/i);
