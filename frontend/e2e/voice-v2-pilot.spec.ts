...
test('spoken and typed input share the canonical V2 recommendation path', async ({ context, page }) => {
  ...
  await trace.getByRole('button', { name: 'Decision', exact: true }).click();
  await trace.getByRole('tab', { name: 'Execution', exact: true }).click();
  ...
  await trace.getByRole('button', { name: 'Audit & Technical', exact: true }).click();
